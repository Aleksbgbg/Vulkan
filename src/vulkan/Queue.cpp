...
CommandPool Queue::CreateCommandPool(const VkCommandPoolCreateFlags createFlags) const {
  return CommandPool(
      device, queue, CommandPoolCreateInfoBuilder().SetFlags(createFlags).SetQueueFamilyIndex(familyIndex));
}

VkResult Queue::Present(Swapchain& swapchain, const u32 imageIndex, const SynchronisationPack& synchronisation) {
