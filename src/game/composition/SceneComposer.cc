...
SceneComposer::SceneComposer(ActorConsumer& actorConsumer, Renderer& renderer,
                             game::Camera& camera)
    : actorConsumer_(actorConsumer), renderer_(renderer), camera_(camera) {}
...
CompositionBuilder SceneComposer::Actor() {
  return CompositionBuilder::ForActor(actorConsumer_, renderer_);
}

CompositionBuilder SceneComposer::Camera() {
  return std::move(
      CompositionBuilder::ForActor(actorConsumer_, renderer_).Attach([this]() {
