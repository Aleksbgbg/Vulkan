...
struct DeviceMemoryObject : public MemoryObject {
  DeviceMemoryObject(DeviceMemory deviceMemory) : deviceMemory(std::move(deviceMemory)) {}

  DeviceMemory deviceMemory;
};

DeviceMemoryAllocator::HeapAllocator::HeapAllocator(VirtualDevice* virtualDevice, u32 memoryTypeIndex)
    : virtualDevice(virtualDevice), memoryTypeIndex(memoryTypeIndex) {}

std::unique_ptr<MemoryObject> DeviceMemoryAllocator::HeapAllocator::Allocate(const VkDeviceSize size) {
  return std::make_unique<DeviceMemoryObject>(virtualDevice->AllocateDeviceMemory(memoryTypeIndex, size));
}

DeviceMemoryAllocator::DeviceMemoryAllocator(VirtualDevice* virtualDevice, const VkPhysicalDeviceMemoryProperties* const memoryProperties)
  : virtualDevice(virtualDevice),
    memoryProperties(memoryProperties),
    heapsPerMemoryIndex(memoryProperties->memoryTypeCount) {
  for (u32 memoryIndex = 0; memoryIndex < heapsPerMemoryIndex.size(); ++memoryIndex) {
...
ReservedMemory DeviceMemoryAllocator::ReserveMemoryBlock(
  ...
  ReservedBlock reservedBlock = heap.ReserveMemory({
    .size = memoryRequirements.size,
    .alignment = memoryRequirements.alignment
  });

  ReservedMemory reservedMemory;
  reservedMemory.memory = &reinterpret_cast<DeviceMemoryObject*>(reservedBlock.GetMemoryObject())->deviceMemory;
