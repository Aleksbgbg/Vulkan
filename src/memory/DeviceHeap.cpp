...
DeviceHeap::DeviceHeap(u64 initialAllocationSize, float enlargementFactor, Allocator* allocator)
...
ReservedBlock DeviceHeap::ReserveMemory(const MemoryAllocation requestedAllocation) {
  while (true) {
    for (AllocatedMemory& allocation : allocations) {
      AllocationList& list = allocation.list;

      AllocationChain* previous = nullptr;
      for (AllocationChain* current = list.First(); current != nullptr; previous = current, current = current->next.get()) {
        const u32 alignedOffset = Align(current->block.offset, requestedAllocation.alignment);
        ...
        if (current->block.size >= alignedSize) {
          ...
          return ReservedBlock(this, allocations[allocatedBlock.allocationIndex].memory.get(), allocationInfo);
        }
      }
    }

    u64 allocationSize = initialAllocationSize * std::ceil(std::pow(enlargementFactor, enlargementIndex));

    if (requestedAllocation.size > allocationSize) {
      const u64 enlargementsRequired = std::log(requestedAllocation.size / initialAllocationSize) / std::log(enlargementFactor);
      const u64 nextEnlargementIndex = enlargementsRequired + 1;
      allocationSize = initialAllocationSize * std::ceil(std::pow(enlargementFactor, nextEnlargementIndex));
    ...
    allocationList.Add(AllocatedBlock{
...
void DeviceHeap::Return(const AllocatedBlock returningBlock) {
  ...
  for (AllocationChain* current = allocation.First(); current != nullptr; current = current->next.get()) {
    const bool comesBeforeCurrentBlock = returningBlock.End() == current->block.offset;
    const bool comesAfterCurrentBlock = current->block.End() == returningBlock.offset;
    ...
    if (comesAfterCurrentBlock) {
      ...
      if ((current->next != nullptr) && (current->block.End() == current->next->block.offset)) {
...
void DeviceHeap::AllocationList::Add(const AllocatedBlock block) {
  if (first == nullptr) {
    first = std::make_unique<AllocationChain>(AllocationChain{
        .block = block,
        .next = nullptr
    });
    last = first.get();
  } else {
    last->next = std::make_unique<AllocationChain>(AllocationChain{
        .block = block,
        .next = nullptr
    });
    last = last->next.get();
...
void DeviceHeap::AllocationList::InsertAfter(DeviceHeap::AllocationChain* previous, const AllocatedBlock block) {
  if (previous == nullptr) {
    first = std::make_unique<AllocationChain>(AllocationChain{
        .block = block,
        .next = std::move(first)
    ...
    previous->next = std::make_unique<AllocationChain>(AllocationChain{
        .block = block,
        .next = std::move(previous->next)
    });
    if (last == previous) {
...
void DeviceHeap::AllocationList::Remove(DeviceHeap::AllocationChain* block, DeviceHeap::AllocationChain* previous) {
  if (previous == nullptr) {
